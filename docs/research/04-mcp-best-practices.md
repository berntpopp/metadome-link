...
- [ ] **Always paginate the landscape.** `get_tolerance_landscape(transcript_id, position_start=, position_stop=, limit=, offset=)`. Default to a region or a hard `limit` (e.g. 200 positions) — never dump 2000 residues unbidden.
