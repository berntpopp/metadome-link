...
from metadome_link import app as app_module
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    monkeypatch.setattr(
        settings,
        "allowed_hosts",
        ["localhost", "127.0.0.1", "::1", "metadome-link.example.org"],
    )
    monkeypatch.setattr(
        settings,
        "allowed_origins",
        ["https://metadome-link.example.org"],
    )
...
@pytest.mark.parametrize("host", ["metadome-link.example.org", "metadome-link.example.org:8443"])
...
@pytest.mark.parametrize("origin", [None, "https://metadome-link.example.org"])
...
def test_default_empty_origin_allowlist_rejects_present_origin(
    ...
    default_client = TestClient(app_module.create_app(), raise_server_exceptions=False)
...
def test_allowlists_load_from_prefixed_json_environment(
    ...
    monkeypatch.setenv("METADOME_LINK_ALLOWED_HOSTS", '["api.example.org"]')
    monkeypatch.setenv("METADOME_LINK_ALLOWED_ORIGINS", '["https://app.example.org"]')
