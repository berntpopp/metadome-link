"""Contract Truth v1 gate against the live MetaDome MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    metadome_requests: list[str] = []

    async def reject_metadome_request(request: httpx.Request) -> httpx.Response:
        metadome_requests.append(str(request.url))
        raise AssertionError("contract discovery invoked the mocked MetaDome API")

    settings = ServerSettings()
    transport = httpx.MockTransport(reject_metadome_request)
    cache = ResultCache(db_path=str(tmp_path / "cache.sqlite"))
    async with httpx.AsyncClient(transport=transport) as http_client:
        client = MetaDomeClient(settings, client=http_client)
        service = MetaDomeService(client, cache, settings=settings)
        try:
            tools = await create_metadome_mcp(service_factory=lambda: service).list_tools()
        finally:
