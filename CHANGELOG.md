...
- **Migrated the container to CPython 3.14** (`python:3.12-slim` →
  ...
  stages. Superseded the bare Dependabot base bump (#28): shipping a 3.14
...
- **CI now runs a Python matrix (`3.12`, `3.14`)** instead of a single 3.12 job.
  3.12 is the `requires-python` floor; 3.14 is what the container actually ships.
  Testing both ends means a future base-image bump inside the supported range can
  never ship an untested interpreter. The coverage gate runs once, on 3.14.
...
- `container-release.json` needs no change: `data.mode` is `"none"` and
  ...
  content paths for the router's OCI content inspector to check.
...
- Do not expose raw, unscoped ClinVar annotations from position-tolerance responses;
  variant evidence remains provenance-scoped and null homolog mappings are unavailable
  rather than misleading zero-count aggregates.
- Make the vendored README-standard checker resolve the repository name from `origin`,
  so isolated Git worktrees validate their checked-in badge correctly.
