...
def main() -> None:
    """Run the metadome-link MCP server on the stdio transport."""
    # Configure environment BEFORE importing anything that may print to stdout.
    os.environ.setdefault("METADOME_LINK_TRANSPORT", "stdio")
    ...
    try:
        ...
        print(f"ERROR: metadome_link import failed: {exc}", file=sys.stderr)
        sys.exit(1)
